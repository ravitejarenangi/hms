...
export default function EditRolePage({ params }) {
  ...
  const fetchRoleAndPermissions = async () => {
    try {
      ...
      Object.keys(permissionsData.permissionsByCategory).forEach(category => {
        initialExpandedCategories[category] = true;
      });
      setExpandedCategories(initialExpandedCategories);
      
      // Fetch role
      const roleResponse = await fetch(`/api/admin/roles/${id}`);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold mt-2">Edit Role: {name}</h1>
        {isSystem && (
          <p className="text-amber-600 mt-1">
            This is a system role. Some restrictions apply to editing.
          </p>
        )}
      </div>
